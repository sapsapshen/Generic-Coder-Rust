...
Usage: python mac_helper.py --action <action> --payload '<json>'

All 24 actions implemented using pyautogui + mss + pyobjc.
"""
...
# Add parent dir to path for shared module import
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

import pyautogui
import mss

# pyautogui safety: disable fail-safe (Corner-UL emergency stop)
...
def list_displays() -> list:
    if not HAS_PYOBJC:
        return []
    err, displays, _ = CGGetActiveDisplayList(10, None, None)
    if err:
        return []
    result = []
...
def frontmost_app() -> dict:
    ...
    return {
        ...
        "displayName": app.localizedName() or "",
    }
    ...
    for path in ["/Applications", "/System/Applications",
        ...
        for entry in os.listdir(path):
            if entry.endswith(".app"):
                apps.append({"name": entry[:-4], "path": os.path.join(path, entry)})
    return apps
...
def do_screenshot(payload: dict):
    ...
    with mss.mss() as sct:
        if display and isinstance(display, int):
            monitor = sct.monitors[display] if display < len(sct.monitors) else sct.monitors[1]
        else:
...
def do_zoom(payload: dict):
    ...
    # Try to use last screenshot if available
...
# Map key names to pyautogui key strings
KEY_MAP = {
...
# Shift modifier for capital letters and symbols
...
def do_hold_key(payload: dict):
    ...
    parts = [p.strip() for p in text.split("+")]
    for p in parts:
        pyautogui.keyDown(_map_modifier(p) if p in ("cmd", "command", "ctrl", "shift", "alt", "option") else p.lower())
    time.sleep(float(duration))
    for p in reversed(parts):
        pyautogui.keyUp(_map_modifier(p) if p in ("cmd", "command", "ctrl", "shift", "alt", "option") else p.lower())
...
def do_computer_batch(payload: dict):
    ...
    for act in actions:
        ...
        if action_name and action_name in DISPATCH:
            try:
                DISPATCH[action_name](act)
                results.append({"action": action_name, "status": "ok"})
            except Exception as e:
                results.append({"action": action_name, "status": "error", "error": str(e)})
        else:
            results.append({"action": action_name, "status": "error", "error": "unknown action"})
