...
impl RemoteServerConnection {
    ...
    fn connect_via_ssh2_inner(
        ...
        let tcp = if !jump_host.is_empty() {
            ...
            // ssh2 channels operate in non-blocking mode
            let _ = ch.stream(0);
            // turn a channel into a TcpStream, so jump host via ssh2 on
            // Windows will fail here — caller falls back to shell ssh.
            #[cfg(unix)]
            {
                use std::os::unix::io::FromRawFd;
                unsafe { std::net::TcpStream::from_raw_fd(ch.as_raw_fd()) }
            }
            #[cfg(not(unix))]
            {
