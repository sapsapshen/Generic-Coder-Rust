...
// ---------------------------------------------------------------------------
// computer use — delegates to Python Bridge (src/computer_use.rs)
// ---------------------------------------------------------------------------

/// Backward-compat wrapper. Prefer crate::computer_use::screenshot() directly.
pub fn computer_screenshot(region: Option<&[u64]>, display: Option<u64>) -> Result<JsonResult> {
    Ok(crate::computer_use::screenshot(region, display)?)
}

/// Backward-compat wrapper. Prefer crate::computer_use::computer_open() directly.
pub fn computer_open(
    application: Option<&str>,
    target: Option<&str>,
    wait_timeout_ms: Option<u64>,
) -> Result<JsonResult> {
    Ok(crate::computer_use::computer_open(application, target, wait_timeout_ms)?)
}

/// Backward-compat wrapper. Prefer crate::computer_use::computer_action() directly.
pub fn computer_action(
    ...
    Ok(crate::computer_use::computer_action(action, x, y, text, direction, amount, duration)?)
}
