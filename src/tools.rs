...
pub fn code_run(
    ...
    // read stdout in a simple loop
    if let Some(ref mut out_pipe) = child.stdout.take() {
        let mut buf = [0u8; 8192];
        loop {
            if let Some(ref sig) = stop_signal {
            ...
            match out_pipe.read(&mut buf) {
                ...
                Err(e) => {
                    return Err(anyhow!("IO error reading stdout: {}", e));
                }
            }
        }
    }
...
pub fn file_read(
    ...
    let selected: Vec<(usize, String)> = if let Some(kw) = keyword {
        ...
        let c = count.unwrap_or(total).min(total - s + 1);
        all_lines[s - 1..s - 1 + c]
            .iter()
            .enumerate()
...
pub fn content_search(
    ...
    let mut matches = match rg_search(pattern, path, glob_pat, max_results, case_sensitive) {
        Ok(m) if !m.is_empty() => m,
        _ => rust_search(pattern, path, glob_pat, max_results, case_sensitive)?,
    };
...
pub fn file_backup(file_path: &str, task_id: Option<&str>) -> Option<String> {
    ...
    let dst = backup_path(file_path, task_id).ok()?;
...
pub fn expand_file_refs(text: &str, base_dir: Option<&str>) -> Result<String> {
    ...
    for cap in FILE_REF_RE.captures_iter(text) {
        ...
        let replaced = if start_line.is_some() || end_line.is_some() {
            ...
            let s = start_line.unwrap_or(1).max(1).min(lines.len());
